...
// Handle search form submission with search history and pagination
app.get("/explore", async (req, res) => {
  try {
      const query = req.query.query || ''; // Extract search query from request parameters or set to empty string
      const page = parseInt(req.query.page) || 1; // Get the page number from request parameters (default to 1)
      const limit = 10; // Number of results per page
      const skip = (page - 1) * limit;

      // Save the search query in session if not empty
      if (query) {
      ...
      // Your logic to search for games based on the query with pagination
      const searchCriteria = query ? { $text: { $search: query } } : {};
      const games = await gameModel.find(searchCriteria).skip(skip).limit(limit);
      const totalResults = await gameModel.countDocuments(searchCriteria);
      const totalPages = Math.ceil(totalResults / limit);

      // Render the explore games page with the retrieved games and search history
      res.render("exploreGames", {
          games,
